...
interface MyTasksSectionProps {
  events: Event[];
  tasks: Task[];  // ✅ ใช้ tasks ที่ส่งเข้ามา
  currentUser: string;
  onStatusChange?: (taskId: string, newStatus: Task["taskStatus"]) => void; // ✅ แก้ชนิด
  onSubTaskToggle?: (taskId: string, subTaskId: string) => void;
  onNavigateToAllTasks?: (filterContext?: "my" | "all") => void;
  onCreatePersonalTask?: () => void;
}

/** -------------------- Added helper for due date mapping -------------------- */
type Dateish = { startDate?: string; endDate?: string; dueDate?: string };
function effectiveDueDateOf(t: Task): string | undefined {
  // map startAt/endAt -> shape ที่ getEffectiveDueDate ต้องการ
  const dateish: Dateish = {
    startDate: t.startAt ?? undefined,
    endDate: t.endAt ?? undefined,
    // ใช้ endAt เป็น dueDate ตามสมมติฐาน (ถ้ามีฟิลด์ dueDate จริงในอนาคต ค่อยสลับมาใช้ได้)
