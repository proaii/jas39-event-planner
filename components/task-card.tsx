...
export function TaskCard({
    ...
    // fallback handler that uses parent if present, otherwise calls server
    const handleStatusChange = (taskId: string, newStatus: Task["taskStatus"]) => {
        if (onStatusChange) {
            onStatusChange(taskId, newStatus);
            return;
        }
        // fallback: call edit mutation to update task status on server
        editTask.mutate({ taskId, patch: { taskStatus: newStatus } });
    };
    ...
    const handleMainTaskToggle = (checked: boolean) => {
        handleStatusChange(task.taskId, checked ? "Done" : "In Progress");
    };
                ...
                <div className="flex justify-start items-center mt-4">
                    ...
                    <DropdownMenuContent align="end" className="w-32" style={{ zIndex: 9999 }}>
                        ...
                        <DropdownMenuItem
                            ...
                            onClick={(e) => {
                            e.stopPropagation();
                            handleStatusChange(task.taskId, st); // <- use fallback handler
                            }}
